...
def _authenticate(client_id: str, client_secret: str, timeout: int,
...
def _graphql_request(graphql_url: str, token: str, query: str,
...
def _parse_page_size(raw_page_size, default):
    try:
        if raw_page_size:
...
class Wiz:
    ...
    def test_connection(self, connectionParameters: dict):
        try:
            ...
            token = _authenticate(client_id, client_secret, timeout, verify_ssl, proxies)
            _graphql_request(graphql_url, token, TEST_CONNECTIVITY_QUERY, {}, timeout, verify_ssl, proxies)
    ...
    def get_issues(self, request: RequestBody) -> dict:
        try:
            ...
            variables = {"first": _parse_page_size(params.get("page_size"), page_size)}
            if params.get("page_token"):
                variables["after"] = params["page_token"]
            if filter_by:
                variables["filterBy"] = filter_by

            data = _graphql_request(graphql_url, token, GET_ISSUES_QUERY, variables, timeout, verify_ssl, proxies)
    ...
    def get_issue_by_id(self, request: RequestBody) -> dict:
        try:
            ...
            params = request.parameters or {}
            issue_id = _validate_required(params.get("issue_id"), "issue_id")

            data = _graphql_request(graphql_url, token, GET_ISSUE_BY_ID_QUERY, {"id": issue_id}, timeout, verify_ssl, proxies)
    ...
    def get_issue_evidence(self, request: RequestBody) -> dict:
        try:
            ...
            data = _graphql_request(graphql_url, token, GET_ISSUE_EVIDENCE_QUERY, {"id": issue_id}, timeout, verify_ssl, proxies)
    ...
    def get_resources(self, request: RequestBody) -> dict:
        try:
            ...
            data = _graphql_request(graphql_url, token, GET_RESOURCES_QUERY, variables, timeout, verify_ssl, proxies)
    ...
    def get_resource(self, request: RequestBody) -> dict:
        try:
            ...
            if resource_id:
                data = _graphql_request(graphql_url, token, GET_RESOURCE_QUERY, {"id": resource_id.strip()}, timeout, verify_ssl, proxies)
                return {"status": "success", "resource": data.get("resource", {})}
            else:
                variables = {"first": 1, "filterBy": {"search": resource_name.strip()}}
                data = _graphql_request(graphql_url, token, GET_RESOURCE_BY_NAME_QUERY, variables, timeout, verify_ssl, proxies)
    ...
    def get_project_team(self, request: RequestBody) -> dict:
        try:
            ...
            params = request.parameters or {}
            project_name = _validate_required(params.get("project_name"), "project_name")

            variables = {"filterBy": {"name": project_name}}
            data = _graphql_request(graphql_url, token, GET_PROJECT_TEAM_QUERY, variables, timeout, verify_ssl, proxies)
