...
def _subscription_list_url(base_url: str, subscription_id: str) -> str:
    return (
        ...
        f"/Microsoft.Network/FrontDoorWebApplicationFirewallPolicies"
        f"?api-version={API_VERSION}"
    )


# ------------------------------------------------------------------
# Policy rule helpers — Front Door schema (matchVariable / matchValue)
# ------------------------------------------------------------------
...
def _find_ip_match_condition(rule: dict) -> Optional[dict]:
    for cond in rule.get("matchConditions", []):
        if (
            cond.get("matchVariable") == "RemoteAddr"
...
def _build_soar_rule(rule_name: str, match_value: list, priority: int = 10) -> dict:
    return {
        "name": rule_name,
        "priority": priority,
        "ruleType": "MatchRule",
...
def _find_url_match_condition(rule: dict) -> Optional[dict]:
    for cond in rule.get("matchConditions", []):
        if (
            cond.get("matchVariable") == "RequestUri"
...
def _build_soar_url_rule(rule_name: str, match_value: list, priority: int = 10) -> dict:
    return {
        "name": rule_name,
        "priority": priority,
        "ruleType": "MatchRule",
...
class AzureWafFrontDoor:
    ...
    def test_connection(self, connectionParameters: dict):
        try:
            ...
            return {"status": "success", "message": "Connected to Azure Front Door WAF successfully."}
        except Exception:
    ...
    def get_custom_rules(self, request: RequestBody) -> dict:
        try:
            ...
            policy = _request_with_refresh("GET", _policy_url(base_url, sub_id, rg, policy_name), tenant_id, client_id, client_secret, timeout, verify_ssl, proxies)
            custom_rules = policy.get("properties", {}).get("customRules", {}).get("rules", [])
    ...
    def add_ip_to_block_list(self, request: RequestBody) -> dict:
        try:
            ...
            policy = _request_with_refresh("GET", _policy_url(base_url, sub_id, rg, policy_name), tenant_id, client_id, client_secret, timeout, verify_ssl, proxies)
            rules_container = policy.setdefault("properties", {}).setdefault("customRules", {})
            custom_rules = rules_container.setdefault("rules", [])
            ...
            if soar_rule is None:
                ...
                if ip_address not in cond["matchValue"]:
                    cond["matchValue"].append(ip_address)
            ...
            updated_rule = _find_soar_rule(policy["properties"]["customRules"]["rules"], rule_name)
    ...
    def remove_ip_from_block_list(self, request: RequestBody) -> dict:
        try:
            ...
            cond = _find_ip_match_condition(soar_rule)
            if cond is None or ip_address not in cond["matchValue"]:
                raise Exception(f"IP '{ip_address}' not found in rule '{rule_name}'.")

            cond["matchValue"].remove(ip_address)
            if not cond["matchValue"]:
    ...
    def check_ip_in_block_list(self, request: RequestBody) -> dict:
        try:
            ...
            cond = _find_ip_match_condition(soar_rule)
            blocked = cond is not None and ip_address in cond.get("matchValue", [])
    ...
    def get_blocked_ips(self, request: RequestBody) -> dict:
        try:
            ...
            cond = _find_ip_match_condition(soar_rule)
            blocked_ips = list(cond.get("matchValue", [])) if cond else []
    ...
    def block_url(self, request: RequestBody) -> dict:
        try:
            ...
            if soar_rule is None:
                ...
                if url_path not in cond["matchValue"]:
                    cond["matchValue"].append(url_path)
    ...
    def unblock_url(self, request: RequestBody) -> dict:
        try:
            ...
            cond = _find_url_match_condition(soar_rule)
            if cond is None or url_path not in cond["matchValue"]:
                raise Exception(f"URL '{url_path}' not found in rule '{rule_name}'.")

            cond["matchValue"].remove(url_path)
            if not cond["matchValue"]:
                soar_rule["state"] = "Disabled"
