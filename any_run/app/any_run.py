...
# Configurable auth header — adjust when official docs confirmed
AUTH_HEADER_NAME = "Authorization"
AUTH_HEADER_PREFIX = ""  # No prefix; raw key. Change to "Bearer " or "ANYRUN-API " as needed

# Centralized endpoint paths — TBD pending official docs
...
class AnyRun:
    ...
    def _make_request(self, method: str, url: str, api_key: str, timeout: int,
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if resp.status_code == 200:
                ...
                return {"_raw_content": base64.b64encode(resp.content).decode("utf-8"),
                        ...
                        "_filename": self._extract_filename(resp)}
    ...
    def _extract_filename(self, resp) -> str:
    ...
    def test_connection(self, connectionParameters: dict):
        try:
            ...
            result = self._make_request("GET", url, api_key, timeout, verify_ssl, proxies)
    ...
    def submit_file(self, request: RequestBody) -> dict:
        try:
            ...
            file_content = self._validate_required(params.get("file_content"), "file_content")
            file_name = self._validate_required(params.get("file_name"), "file_name")
            ...
            result = self._make_request("POST", url, api_key, timeout, verify_ssl, proxies,
    ...
    def submit_url(self, request: RequestBody) -> dict:
        try:
            ...
            url_to_analyze = self._validate_required(params.get("url"), "url")
            ...
            result = self._make_request("POST", endpoint, api_key, timeout, verify_ssl, proxies,
                                        json_data=payload)
            task_id = result.get("data", {}).get("task_id") or result.get("task_id", "")
            return {"status": "success", "task_id": task_id, "raw_response": result}
        except Exception as e:
    ...
    def get_analysis_status(self, request: RequestBody) -> dict:
        try:
            ...
            task_id = self._validate_required(params.get("task_id"), "task_id")
    ...
    def download_artifact(self, request: RequestBody) -> dict:
        try:
            ...
            task_id = self._validate_required(params.get("task_id"), "task_id")
            artifact_type = self._validate_required(params.get("artifact_type"), "artifact_type")
